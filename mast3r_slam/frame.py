...
class SharedStates:
    ...
    def set_save_state(self, value):
        with self.lock:
            self.save_state_flag.value = 1 if value else 0

    def should_save_state(self):
        with self.lock:
            return self.save_state_flag.value == 1
...
class SharedKeyframes:
    def __init__(self, manager, h, w, buffer=512, dtype=torch.float32, device="cuda"):
        ...
        self.relocalized_flag = manager.Value("i", 0)  # Flag to track relocalization from backend
