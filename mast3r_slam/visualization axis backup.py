...
from mast3r_slam.coord_transform import calculate_world_tilt_from_config
...
class Window(WindowEvents):
    ...
    def __init__(self, states, keyframes, main2viz, viz2main, **kwargs):
        ...
        # Create a second axis with your inverted rotations applied
        self.axis_ground_frame = Axis(self.line_prog, 0.1, 3 * self.scale)
    ...
    def render(self, t: float, frametime: float):
        ...
        if self.show_axis:
            ...
            # 1. Calculate the actual tilt that occurred in the world frame.
            R_equivalent_world_tilt = calculate_world_tilt_from_config(config)

            # 2. Get the original vertices of the ground frame axis.
            original_lines = self.axis_ground_frame.lines.reshape(-1, 4)
            transformed_lines = original_lines.copy()
            
            # 3. Define an offset vector to shift the tilted axis for clarity.
            # This will move the axis 0.5 units along the world's positive X-axis.
            offset_vector = np.array([0.5, 0.0, 0.0])

            # 4. Apply the WORLD FRAME tilt and the OFFSET to the WORLD FRAME points.
            for i in range(len(original_lines)):
                point_to_rotate = original_lines[i][:3]
                
                # First, apply the rotation
                rotated_point = R_equivalent_world_tilt @ point_to_rotate
                
                # Then, add the offset vector to the rotated point
                final_point = rotated_point + offset_vector
                
                transformed_lines[i][:3] = final_point
            
            # 5. Render the transformed axis by temporarily swapping its vertex data.
            original_lines_backup = self.axis_ground_frame.lines.copy()
            self.axis_ground_frame.lines = transformed_lines.flatten()
            self.axis_ground_frame.render(self.camera)
            self.axis_ground_frame.lines = original_lines_backup # Restore for next frame
        ...
        # Check if we should show a target keyframe in blue
        show_target = False
        target_kf_idx = -1
        try:
            with self.states.lock:
                show_target = bool(self.states.show_target_keyframe.value)
        ...
        for kf_idx in range(N_keyframes):
            ...
            # Determine color: blue for target keyframe if flag is set, red otherwise
            if show_target and kf_idx == target_kf_idx:
                color = [0, 0, 1, 1]  # Blue for target keyframe
            else:
                color = [1, 0, 0, 1]  # Red for other keyframes
